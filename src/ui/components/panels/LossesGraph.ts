...
/**
 * LossesGraph: Running tally of destroyed units and expended munitions.
 */
export class LossesGraph extends Component {
    ...
    protected styles() {
        return `
        .losses-widget { padding:var(--sp-3); }
        .lg-title { font-size:var(--text-xs); font-weight:600; color:var(--text-muted); text-transform:uppercase; margin-bottom:var(--sp-2); }
        .lg-canvas { width:100%; height:120px; background:var(--bg-surface); border:1px solid var(--border-color); border-radius:var(--radius-sm); }
        .lg-stats { display:flex; gap:var(--sp-4); margin-top:var(--sp-2); font-size:var(--text-xs); font-family:var(--font-mono); }
        .lg-stat { display:flex; flex-direction:column; gap:2px; }
        .lg-stat__label { color:var(--text-dim); text-transform:uppercase; }
        .lg-stat__value { font-size:var(--text-sm); font-weight:600; }
