...
/**
 * LoadoutWindow: Aircraft loadout editor affecting weight/drag/RCS.
 * Ported to V2 WindowManager architecture.
 */
export class LoadoutWindow extends Component {
    constructor() { super('div', 'loadout-widget'); }

    protected styles() {
        return `
        .loadout-widget { padding: var(--sp-3); display: flex; flex-direction: column; gap: var(--sp-2); height: 100%; box-sizing: border-box; overflow-y: auto; }
        .lo-title { font-size: var(--text-xs); font-weight: 700; color: var(--text-muted); text-transform: uppercase; margin-bottom: var(--sp-2); letter-spacing: 0.05em; }
        .lo-preset { display: flex; flex-direction: column; gap: var(--sp-2); }
        .lo-card { padding: var(--sp-2) var(--sp-3); background: var(--bg-base); border: 1px solid var(--border-color); border-radius: var(--radius-md); cursor: pointer; transition: all var(--transition-fast); }
        ...
        .lo-card__name { font-size: var(--text-sm); font-weight: 600; color: var(--text-main); }
        .lo-card__desc { font-size: var(--text-xs); color: var(--text-muted); margin-top: 2px; }
        .lo-card__stats { display: flex; gap: var(--sp-3); margin-top: var(--sp-1); font-size: var(--text-xs); font-family: var(--font-mono); color: var(--text-dim); }
        .lo-station { display: grid; grid-template-columns: 60px 1fr; gap: 4px; padding: var(--sp-1) 0; font-size: var(--text-xs); border-bottom: 1px solid var(--border-color); align-items: center; }
        .lo-station__label { color: var(--text-dim); font-weight: 600; }
        .lo-station__select { background: var(--bg-hover); border: 1px solid var(--border-color); border-radius: 2px; padding: 4px; font-size: var(--text-xs); color: var(--text-main); width: 100%; outline: none; }
        .lo-station__select:focus { border-color: var(--color-friendly); }
        `;
    }
