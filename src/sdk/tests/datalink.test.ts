...
import { ViewStatePayload, Side } from '../schemas';
...
describe('SDK Datalink Serialization', () => {
    ...
    it('should preserve datalink graph during encode/decode', () => {
        const mockState: ViewStatePayload = {
            ...
            datalinkGraph: {
                nodes: ['unit-1', 'unit-2'],
                edges: [
                    { a: 'unit-1', b: 'unit-2', latencyMs: 50 }
                ]
            },
        ...
        const decoded = decoder.decode(encoded.buffer as ArrayBuffer);

        expect(decoded.datalinkGraph.nodes).toContain('unit-1');
        expect(decoded.datalinkGraph.nodes).toContain('unit-2');
        expect(decoded.datalinkGraph.edges).toHaveLength(1);
        expect(decoded.datalinkGraph.edges[0]).toMatchObject({ a: 'unit-1', b: 'unit-2', latencyMs: 50 });
    });
    ...
    it('should handle empty datalink graph', () => {
        ...
        expect(decoded.datalinkGraph.nodes).toHaveLength(0);
